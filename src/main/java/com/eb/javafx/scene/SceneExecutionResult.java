 ...
 * optional hotspot map view model, rollback availability, and any terminal or diagnostic message produced while
 * advancing the scene.</p>
 */
public final class SceneExecutionResult {
    ...
    public SceneExecutionResult(SceneExecutionStatus status, SceneFlowState state, SceneStep step,
            List<SceneChoice> availableChoices, HotspotMapViewModel hotspotMapViewModel, String message, boolean canRollback) {
        this.status = status;
        this.state = state;
        this.step = step;
    ...
    public SceneExecutionResult(SceneExecutionStatus status, SceneFlowState state, SceneStep step,
        ...
        this(status, state, step, availableChoices, null, message, canRollback);
    }

    public SceneExecutionResult(SceneExecutionStatus status, SceneFlowState state, SceneStep step,
            List<SceneChoice> availableChoices, HotspotMapViewModel hotspotMapViewModel, String message) {
        this(status, state, step, availableChoices, hotspotMapViewModel, message, false);
    }

    public SceneExecutionResult(SceneExecutionStatus status, SceneFlowState state, SceneStep step,
            List<SceneChoice> availableChoices, String message) {
        this(status, state, step, availableChoices, null, message, false);
    }
