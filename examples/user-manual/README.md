...
| `06-ui-screens-and-themes/FooterControlDemo.java` | Wiring the standard footer onto a custom screen shell with `FooterController` — id-based click router for back/forward/skip/history plus the F5/F9 quick-save shortcuts. | Reference snippet showing the `wireFooter` + `installKeyboardShortcuts` calls; not run directly (placeholder `RouteContext`/story node supplied by the host). |
...
| `09-game-support-state-save-prefs-random/EventsDemo.java` | The events package: fire-and-forget bus, FIFO queue, command dispatch, and the completion-gated `EventSequencer` (turn/bonus ordering). | Prints the bus event and history size, drained queue count, command-emitted events, and the sequencer order `[turnEndA, turnEndB, bonus, BARRIER, turn2]`. |
