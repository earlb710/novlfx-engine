...
- `UtilConvert`, `UtilString`, `UtilUnicode`, and `ECharMappings` for small conversion, string, Unicode, and character-mapping helpers retained for reusable engine code. `UtilString.titleCase(s)` capitalizes the first character (Unicode-aware, null/empty-safe) — handy for turning a lower-case nickname or id into a display label. `UtilString.appendUniqueToken(list, token, divider)` appends a token to a divider-separated list, skipping it when the exact token is already present (dedupe); `UtilString.appendSignedPart(parts, label, value)` appends a signed `"label +N"` / `"label -N"` entry (skipping zero) for building compact effect summaries. `UtilString.spacedTitleCase(text, separator)` splits on a separator, capitalises each word, and joins with spaces (`"early-morning", '-'` → `"Early Morning"`); `UtilString.dayOfWeekName(day)` maps a 1-based day counter (1 = Monday) to its English weekday name, wrapping weekly.
...
| `com.eb.javafx.util.ColorCss` | The `javafx.scene.paint.Color`-typed companion to `ThemeColors`: `luminance` (Rec.601), `isLight` (dark-text test), `withAlpha`, `lerp(a, b, t)` (linear RGB interpolation), `toRgbaCss` (Color → `rgba(r,g,b,a)`), `formatAlpha(alpha)` (locale-safe `%.2f` alpha token, clamped), `backgroundColorStyle(base, alpha)` (an `-fx-background-color: rgba(...)` inline style), `labelStyle(text, size, weight, bg)` (inline `-fx-` label style), plus `clamp01` / `to255` / `clampByte255` component clamps. |
...
| `com.eb.javafx.ui.ModalOverlayHost` | Stateless modal-overlay building blocks (sits beside `PanelPopup`): `centredCard()` (content-sized card that consumes its own clicks), `dimBackdrop(card, dimColor)` (hidden dim full-screen backdrop), `scrollWrap(content, styleClasses…)` (transparent AS_NEEDED fit-to-width scroll wrapper), `firstDescendantOfType(root, type)` / `firstDescendantWithStyleClass(root, styleClass)` (depth-first scene-graph finders — e.g. the `ScrollPane` to snapshot/restore a rebuilt popup's scroll position, or a style-class-marked wrapper node), and `tintScrollChrome(region, color, alpha)` (paints a ScrollPane-skinned region's outer chrome AND its `.viewport` together). The host wires its own visibility / dismissal / policy around these. Example: `StackPane card = ModalOverlayHost.centredCard(); StackPane overlay = ModalOverlayHost.dimBackdrop(card, "rgba(0,0,0,0.55)"); overlay.setOnMouseClicked(e -> hide());` |
| `com.eb.javafx.ui.HoverChip` | `build(icon, label, restStyle, hoverStyle, idPrefix, onClick)` — a small clickable chip (icon glyph + bold label) with a rest/hover background swap, hand cursor, and an optional stable id (`<idPrefix>-<label>`, spaces→`-`) for automation. Rest/hover styles are caller-supplied so it carries no game-specific look. |
| `com.eb.javafx.ui.ClockSweepOverlay` | `play(parent, caption, fromAngle, target, steps, totalMillis, sleeping, sleepButtonSkin, sleepAction, onSleepDissolve, onFinished)` — a game-agnostic "time passes" analog-clock overlay: the hour hand sweeps `fromAngle → target` (>360 allowed for an overnight revolution) over `steps` ticks on a dim modal backdrop, then dissolves and runs `onFinished` (or runs it immediately when `parent` is null). Policy is injected: the Sleep-button skin, the pre-dissolve cleanup, and the finish continuation. `TURN_STEPS`/`TURN_MILLIS` (60/3000) and `SLEEP_STEPS`/`SLEEP_MILLIS` (100/5000) are the standard turn/overnight cadences. Example: `ClockSweepOverlay.play(host, "Waiting…", 90, 180, ClockSweepOverlay.TURN_STEPS, ClockSweepOverlay.TURN_MILLIS, false, myButtonSkin, null, null, this::onClockDone);` |
