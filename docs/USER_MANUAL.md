...
| `com.eb.javafx.ui.ModalOverlayHost` | Stateless modal-overlay building blocks (sits beside `PanelPopup`): `centredCard()` (content-sized card that consumes its own clicks), `dimBackdrop(card, dimColor)` (hidden dim full-screen backdrop), `scrollWrap(content, styleClasses…)` (transparent AS_NEEDED fit-to-width scroll wrapper), and `firstDescendantOfType(root, type)` (depth-first scene-graph finder — e.g. the `ScrollPane` to snapshot/restore a rebuilt popup's scroll position). The host wires its own visibility / dismissal / policy around these. Example: `StackPane card = ModalOverlayHost.centredCard(); StackPane overlay = ModalOverlayHost.dimBackdrop(card, "rgba(0,0,0,0.55)"); overlay.setOnMouseClicked(e -> hide());` |
