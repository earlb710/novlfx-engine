...
- `choice`: treats each variant as a player-selectable choice. Each choice variant can carry its own `conditions` array; the runtime projection stores those conditions with the generated scene choice metadata.
...
```json
{
  ...
  "type": "choice",
  "variants": [
    {"text": "Take the left path.", "weight": 1.0, "conditions": ["has_key"]},
